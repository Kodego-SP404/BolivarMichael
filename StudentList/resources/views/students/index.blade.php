   ...
   <div class="overflow-x-auto relative ">
      <table class="w-96 mx-auto text-sm text-left text-gray-500 ">
         ...
         <tbody>
            ...
            <tr class="bg-stone-900 border-b text-white bg-opacity-50">
               <td class="py-4 px-6">
                  {{ $student-> first_name }}
               </td>
               <td class="py-4 px-6">
               ...
               <td class="py-4 px-6">
                  <a href="/student/{{$student->id}}" class="bg-amber-600 hover:bg-amber-800 text-white px-4 py-1 rounded">view</a>
                  <form action="/student/{{$student->id}}" method="POST">
                     @method('delete')
                     @csrf
                     <button type="submit" class="w-full mt-2 bg-red-600 hover:bg-amber-800 text-white py-1 rounded shadow-lg hover:shadow-xl transitionduration-200" type="submit">Delete</button>
                  </form>
               </td>            
            </tr>
            @endforeach
